...
export const dateDiff = (val) => {
    ...
    if (diff > range1[0] && diff <= range1[1]) {
        return '刚刚';
    } if (diff > range2[0] && diff <= range2[1]) {
        diff = Math.floor(diff);
        return `${diff}分钟前`;
    } if (diff > range3[0] && diff <= range3[1]) {
        diff = Math.floor(diff / 60);
        return `${diff}小时前`;
    } if (diff > range4[0] && diff <= range4[1]) {
    ...
    } if (diff > range5[0] && diff <= range5[1]) {
        return `${formatDate(m, 'M月D日 HH:mm')}`;
    } if (diff > range6[0] && diff <= range6[1]) {
